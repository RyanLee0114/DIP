...
class KioskApp:
    ...
    def show_main_menu(self):
        ...
        # Bottom frame for checkout button
        bottom_frame = tk.Frame(right_frame, bg='skyblue')
        ...
        tk.Button(bottom_frame, text="Check Out", bg='orange', font=('Arial', 14), command=self.show_dine_option).pack(pady=10)
    ...
    def show_cart(self):
        ...
        def update_item_total_label(lbl, item, qty):
            lbl.config(text=f"= ${item['price'] * qty}")

        def delete_item(item_name):
            # Remove all items with this name from cart
            removed_qty = item_counts[item_name]['qty']
            item = item_counts[item_name]['item']
            self.cart = [cart_item for cart_item in self.cart if cart_item['name'] != item_name]
            self.total_price -= item['price'] * removed_qty
            # Remove from item_counts
            del item_counts[item_name]
            # Refresh cart screen
            self.show_cart()
        ...
        for item_name, data in item_counts.items():
            ...
            def update_qty(name=item_name, v=var, lbl=item_total_label):
                ...
                update_total_label()
                update_item_total_label(lbl, item, new_qty)
            ...
            # Add delete button
            del_btn = tk.Button(frame, text="Delete", font=('Arial', 12), bg='red', fg='white',
                                command=lambda name=item_name: delete_item(name))
